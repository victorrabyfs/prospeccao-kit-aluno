...
/** "Gov. Valadares" = "Governador Valadares"; sem acento, sem caixa, sem a UF no fim. */
const cidadeNormal = (c) => String(c ?? '').normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase()
  .replace(/\s+[a-z]{2}$/, '').replace(/\bgov\.?\s/, 'governador ').replace(/\bcel\.?\s/, 'coronel ')
  .replace(/\s+/g, ' ').trim();
...
export function checarRequisitos(lead, regras, repetidos = new Set()) {
  ...
  // A busca do Maps casa o nome da cidade com o nome da rua: "clínica em Governador Valadares" traz a
  // clínica da Av. Gov. Valadares em Juiz de Fora. Com `cidades` no nicho, lead de outra cidade sai.
  if (Array.isArray(regras.cidades) && regras.cidades.length && lead.cidade
      && !regras.cidades.some((c) => cidadeNormal(c) === cidadeNormal(lead.cidade))) {
    motivos.push(`fora das cidades do nicho (${lead.cidade})`);
  }
