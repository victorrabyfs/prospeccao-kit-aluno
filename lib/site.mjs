...
const DOMINIO_A_VENDA = /(this domain (is|may be) for sale|domain is for sale|hugedomains|sedo\.com|dan\.com|domínio (está )?à venda|este domínio pode estar à venda|parked (domain|free)|registro\.br.{0,40}dispon[ií]vel)/i;
...
export async function enriquecerSite(site) {
  ...
  // Domínio à venda ou estacionado responde 200, mas a empresa não tem site: é site fora do ar.
  if (DOMINIO_A_VENDA.test(base.texto.slice(0, 3000))) {
    return {
      site_status: principal.status, site_url_final: principal.url_final,
      site_vivo: false, site_erro: 'o domínio está à venda ou estacionado',
      emails: [], telefones_site: [], whatsapp: null, instagram: null,
      facebook: null, linkedin: null, youtube: null, cnpj: null, site_titulo: base.titulo, texto: '',
      marketing: null,
    };
  }
  const combinado = {
